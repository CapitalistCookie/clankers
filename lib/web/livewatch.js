...
(function (root, factory) {
  ...
  // ── UplinkTracker — liveness of the CLIENT -> SERVER direction ─────────────
  // The stall watchdog above judges the DOWNSTREAM only, and it counts any
  // inbound frame as proof of life. On a pane that is actively printing,
  // content frames never stop, so an uplink that has gone dead is invisible to
  // it forever — which is exactly how a session could stream updates while
  // every message typed into it vanished (2026-09-10).
  //
  // Every frame we send that MATTERS carries a seq; the server echoes it (ack
  // for input, pong for keepalive). An echo is the OUTCOME — the server really
  // received it — where readyState is only a proxy, and a proxy that stays
  // OPEN forever on a half-open socket.
  //
  // opts: {now, timeoutMs, deadAfter}
  //   timeoutMs — how long one un-echoed seq may sit before it counts as lost
  //   deadAfter — consecutive losses before the uplink is declared dead
  function createUplinkTracker(opts) {
    opts = opts || {};
    var now = opts.now;
    var timeoutMs = opts.timeoutMs == null ? 6000 : opts.timeoutMs;
    var deadAfter = opts.deadAfter == null ? 2 : opts.deadAfter;
    var seq = 0, outstanding = [], consecutiveLost = 0;
    return {
      // Register a frame we just handed to the socket. `meta` is whatever the
      // caller needs back on failure (e.g. the message text to restore).
      track: function (meta) {
        seq++;
        outstanding.push({ seq: seq, at: now(), meta: meta });
        return seq;
      },
      // Server echoed a seq. Returns the tracked entry, or null when the seq is
      // unknown (a late echo for something we already gave up on — the caller
      // must NOT treat that as a fresh success).
      settle: function (n) {
        for (var i = 0; i < outstanding.length; i++) {
          if (outstanding[i].seq !== n) continue;
          var entry = outstanding.splice(i, 1)[0];
          consecutiveLost = 0;
          return entry;
        }
        return null;
      },
      // Entries past their deadline. Returned ONCE, then forgotten: the caller
      // owns the failure from here (restore the text, warn, reconnect).
      // `all` drains everything regardless of deadline — for socket teardown,
      // where nothing outstanding can ever be echoed. A drain does NOT count
      // toward the dead() verdict: the socket is going away on purpose.
      sweep: function (all) {
        var cutoff = now() - timeoutMs, lost = [], keep = [];
        for (var i = 0; i < outstanding.length; i++) {
          if (all || outstanding[i].at <= cutoff) lost.push(outstanding[i]);
          else keep.push(outstanding[i]);
        }
        outstanding = keep;
        if (lost.length && !all) consecutiveLost += lost.length;
        return lost;
      },
      // True once enough round trips have been lost in a row that the uplink
      // cannot be given the benefit of the doubt.
      dead: function () { return consecutiveLost >= deadAfter; },
      pending: function () { return outstanding.length; },
      lost: function () { return consecutiveLost; },
      // Socket rebuilt: seqs belong to the old one, so nothing outstanding can
      // ever be echoed. Caller sweeps first if it wants the failures.
      reset: function () { outstanding = []; consecutiveLost = 0; }
    };
  }
  ...
  return {
    ...
    createUplinkTracker: createUplinkTracker
  };
});
