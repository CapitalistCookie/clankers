...
// Attention priority (items 2+4): the thing most needing you sorts to the top.
// A blocking decision outranks an error outranks a limit outranks a plain
// finished turn outranks a fan-out outranks a compaction outranks routine work.
function attentionRank(s) {
  const sub = s.substate;
  if (sub === 'decision') return 0;
  if (sub === 'error') return 1;
  if (sub === 'limit') return 2;
  if (s.state === 'waiting') return 3;
  if (sub === 'subagents') return 4;
  if (sub === 'compacting') return 5;
  return 6;   // routine working / idle
}
// Favourites still pin to the very top; within each group, attention then name.
...
// Chips derive purely from the /api/status payload (substate/agents/ctx/b5h/b7d)
// — no extra fetch, so A5's change-detection still gates the re-render.
function sessionChips(s) {
  const chips = [];
  const sub = s.substate;
  if (sub === 'decision') chips.push('<span class="s-chip red">⚠ decision</span>');
  else if (sub === 'error') chips.push('<span class="s-chip red">error</span>');
  else if (sub === 'limit') chips.push('<span class="s-chip amber">limit</span>');
  else if (sub === 'subagents') {
    const n = s.agents || 1;
    chips.push('<span class="s-chip green">⚙ ' + n + ' agent' + (n > 1 ? 's' : '') + '</span>');
  } else if (sub === 'compacting') chips.push('<span class="s-chip dim">compacting</span>');
  // A session with no pane is a `claude` background job: real, but not in tmux,
  // so it carries no terminal — say so on the card instead of leaving a dead click.
  if (!s.target) chips.push('<span class="s-chip dim">bg job</span>');
  if (typeof s.ctx === 'number')
    chips.push('<span class="s-chip ' + (s.ctx >= 85 ? 'red' : s.ctx >= 70 ? 'amber' : '')
      + '">ctx ' + s.ctx + '%</span>');
  if (typeof s.b5h === 'number')
    chips.push('<span class="s-chip ' + (s.b5h >= 80 ? 'amber' : '') + '">5h ' + s.b5h + '%</span>');
  if (typeof s.b7d === 'number')
    chips.push('<span class="s-chip ' + (s.b7d >= 80 ? 'amber' : '') + '">7d ' + s.b7d + '%</span>');
  return chips.length ? '<div class="session-chips">' + chips.join('') + '</div>' : '';
}

function renderLiveSessions(sessions) {
  const el = document.getElementById('live-sessions');
  if (!el) return;
  const sorted = sessions.filter(s => s.command === 'claude').sort(attentionSort);
  el.innerHTML = sorted.map(s => `
    <div class="session-card${s.target ? '' : ' no-terminal'}"
         ${s.target ? `onclick="openTerminal('${s.session}')"` : ''}>
      ${favStar(s.session)}
...
function openWaitingSession() {
  // Only a session with a pane can be opened; a waiting bg job has no terminal.
  const s = waitingSessions.find(x => x.target);
  if (s) openTerminal(s.session);
}
...
// ── Stream liveness: judged on DATA ARRIVING, not on readyState ──────────────
// readyState is only a proxy, and it lies: a half-open socket (mobile network
// switch, NAT expiry, tunnel drop) stays OPEN forever with no close event, so
// every onclose-driven path above is blind to it. The server emits a heartbeat
// frame even when the pane is unchanged (serve.py VIEW_HEARTBEAT_SECS), so
// silence past STALL_MS is unambiguous — the stream is dead, rebuild it.
const STALL_MS = 30000;        // > 2x the server heartbeat: one late hb is not death
const STALL_POLL_MS = 5000;
let liveWatch = null, liveWatchTimer = null;

// ── Uplink liveness (2026-09-10) ────────────────────────────────────────────
// liveWatch above proves the DOWNSTREAM only, and it accepts any inbound frame
// as proof. A pane that is printing sends content forever, so a dead uplink is
// invisible to it — the shape of the bug where a session streamed updates while
// every message typed into it disappeared. The tracker below judges the uplink
// by round trips the SERVER echoed, and it is the thing that lets a failed send
// put the operator's text back in the box instead of eating it.
const ACK_TIMEOUT_MS = 6000;   // one send-keys is ~5ms; 6s is a dead path, not a slow one
const ACK_DEAD_AFTER = 2;      // consecutive lost round trips before we rebuild the socket
const ACK_SWEEP_MS = 1000;
let uplink = null, uplinkTimer = null;
let viewAckSupported = false;  // set from the server's meta frame; no guessing

function liveTouch() { if (liveWatch) liveWatch.touch(); }

function armUplink() {
  uplink = window.ClankerLiveWatch.createUplinkTracker({
    now: () => Date.now(), timeoutMs: ACK_TIMEOUT_MS, deadAfter: ACK_DEAD_AFTER,
  });
}

// A lost round trip is a FAILED send. Hand each one back to whoever queued it
// (the compose bar restores the message) and rebuild the socket once the uplink
// has lost enough in a row to be called dead.
function _ackSweep() {
  if (!uplink) return;
  const lost = uplink.sweep();
  for (const entry of lost) onSendFailed(entry.meta, 'no response from server');
  if (uplink.dead()) {
    setTermState('uplink dead — reconnecting');
    uplink.reset();
    forceReconnect();
  }
}

// Single place where a send is known to have failed, whatever proved it.
function onSendFailed(meta, why) {
  if (!meta) return;
  if (meta.kind === 'compose') {
    meta.group[meta.part] = false;
    // Only a failed TEXT frame means nothing reached the pane. A failed Enter
    // means the text is sitting in the pane's input line unsubmitted — putting
    // it back in the box too would duplicate it on the next send.
    if (meta.part === 'text') composeRestore(meta.text, why);
    else composeNote('text sent, submit failed — press ↵', 'err');
  } else if (meta.kind === 'ping') {
    // The reconnect the dead() verdict triggers is the visible outcome; a
    // keepalive is not something the operator asked for, so it stays quiet.
  } else {
    setTermState('input lost — ' + why);
  }
}

function armLiveWatch() {
  disarmLiveWatch();
  armUplink();
  liveWatch = window.ClankerLiveWatch.createStallWatchdog({
    now: () => Date.now(), stallMs: STALL_MS, graceMs: 4000,
  });
  liveWatch.setHidden(document.hidden);
  liveWatchTimer = setInterval(() => {
    if (!liveMode || !liveName || !liveWatch) return;   // terminal closed
    if (liveSocketDead()) { scheduleReconnect(); return; }
    if (liveWatch.stalled()) {
      setTermState('stalled — reconnecting');
      forceReconnect();
    }
  }, STALL_POLL_MS);
  // The uplink is swept far more often than the downstream is polled: a lost
  // message must surface while the operator is still looking at the box they
  // typed it into, not 5 seconds later.
  uplinkTimer = setInterval(_ackSweep, ACK_SWEEP_MS);
}

function disarmLiveWatch() {
  if (liveWatchTimer) { clearInterval(liveWatchTimer); liveWatchTimer = null; }
  if (uplinkTimer) { clearInterval(uplinkTimer); uplinkTimer = null; }
  liveWatch = null;
  // Anything still outstanding belongs to a socket that is going away, so it can
  // never be acked. Report those failures rather than letting them evaporate —
  // silently dropping them is the whole bug this layer exists to prevent.
  if (uplink) {
    for (const entry of uplink.sweep(true)) onSendFailed(entry.meta, 'socket closed');
    uplink = null;
  }
}

// Rebuild the socket REGARDLESS of what readyState claims. Used by the stall
// watchdog and by the foreground-return path.
function forceReconnect() {
  if (!liveMode || !liveName) return;
  if (reconnectTimer) { clearTimeout(reconnectTimer); reconnectTimer = null; }
  const stale = liveWs;
  liveWs = null;              // supersede first: the old socket's late handlers bail
  if (stale) { try { stale.close(); } catch (e) {} }
  reconnectDelay = 600;
  reconnectLive();
}
...
function armKeepalive(ws) {
  const iv = setInterval(() => {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      // Tracked, so the echo is evidence and not just traffic: on an idle pane
      // this is the ONLY thing proving the uplink still carries our frames.
      // (Untracked before 2026-09-10 — the round trip happened and the answer
      // was thrown away.)
      if (ws === liveWs && uplink && viewAckSupported) _wsSendTracked({ type: 'ping' }, { kind: 'ping' });
...
function connectPty(name) {
  ...
  ws.onmessage = (e) => {
    if (liveWs !== ws) return;      // superseded socket
    liveTouch();
    // PTY payload is ALWAYS binary (server: ws.send_bytes), so a TEXT frame is a
    // control message (hb/pong) and must never be written into the terminal.
    if (typeof e.data === 'string') return;
    if (liveTerm) liveTerm.write(new Uint8Array(e.data));
  };
  ws.onclose = () => { if (liveWs !== ws) return; setTermState('disconnected'); scheduleReconnect(); };
  ws.onerror = () => { if (liveWs !== ws) return; setTermState('error'); };
}
...
let viewPending = null, viewCapture = null, viewLastFrame = null;
// Paint scheduling goes through a SELF-HEALING latch (lib/web/livewatch.js), not
// a bare rAF handle. The old `if (viewRaf) return` latched forever when a mobile
// tab was frozen between arming rAF and its callback firing (the browser drops
// the pending callback): every later frame was silently discarded, the socket
// stayed healthy, and only close+reopen recovered. 2026-08-15.
let viewLatch = null;
function _viewPaintLatch() {
  if (!viewLatch) {
    viewLatch = window.ClankerLiveWatch.createPaintLatch({
...
let viewLineCache = [];       // 13a: last frame's per-line "cls\0inner" keys, for DOM diffing
...
function _viewRenderNow() {
  ...
  // 13a: parse the whole frame once (ANSI state carries across lines, and the
  // TUI-bottom chrome needs the full frame), but only WRITE the DOM for lines
  // that actually changed — the per-frame innerHTML rebuild was the cost.
  _patchViewLines(livePre, renderAnsiToLineList(viewPending));
  if (atBottom) livePre.scrollTop = livePre.scrollHeight;
  viewPending = null;
}
// Reconcile livePre's child <div>s against the new per-line list: replace only
// changed lines in place, append/remove on a length change. Each frame line is
// its own element (was one innerHTML blob), so an unchanged line costs nothing.
function _patchViewLines(container, list) {
  const prev = viewLineCache, n = list.length;
  const keys = new Array(n);
  // NUL separator: class strings contain spaces ("tline rule"), so a plain
  // space could alias two different (cls, inner) pairs to the same key.
  const SEP = String.fromCharCode(0);
  for (let i = 0; i < n; i++) keys[i] = list[i].cls + SEP + list[i].inner;
  for (let i = 0; i < n; i++) {
    if (i < prev.length && prev[i] === keys[i]) continue;   // unchanged — no DOM write
    const div = document.createElement('div');
    div.className = list[i].cls;
    div.innerHTML = list[i].inner;
    const cur = container.children[i];
    if (cur) container.replaceChild(div, cur);
...
function _viewSchedule() {
  ...
  // Coalescing is the latch's job, and it re-arms rather than latching when a
  // scheduled callback never fires.
  if (document.hidden) return;
  _viewPaintLatch().schedule(_viewRenderNow);
}
...
function viewScroll(dir) {
  ...
  wsScrollKey(dir === 'up' ? 'PPage' : 'NPage');
...
function jumpToLive() {
  if (viewAppScrolled) for (let i = 0; i < 14; i++) wsScrollKey('NPage');
  _viewSnapLive();
}

// ── wheel/drag scroll of a TUI's own transcript.
//
// This used to send raw SGR mouse-wheel reports (ESC [ < 64;1;1 M, up to 16 per flick)
// with `send-keys -l`, which is LITERAL injection into the pane's stdin. It was gated on
// `viewIsTui`, which is tmux's `#{alternate_on}` -- the alt screen -- and that is not the
// same thing as "the application has mouse tracking on". Claude Code toggles mouse mode
// off while staying on the alt screen, and whenever it did, those bytes missed the mouse
// parser and were typed into the input field: readline eats `ESC [ <` as an unmatched
// prefix and inserts the rest, so one flick left up to 16 copies of `65;1;1M` -- about 112
// characters -- in whatever the operator was writing. Root-caused 2026-09-17 after Josh
// reported "random repeating characters" appearing while he was sshed in; the web tab and
// the ssh view are the same pane, and a phone with the tab open spams it through
// `touchmove`. tmux 3.5 exposes no format for an application's mouse mode, so there is no
// way to make that gate correct.
//
// A named key cannot be mistyped this way: `{type:'key'}` goes through send-keys WITHOUT
// `-l`, so tmux resolves it as a key rather than as bytes. The cost is granularity -- a
// flick is a page rather than n lines -- and that is the right trade against corrupting
// what somebody is typing. `send-keys WheelUpPane` was measured on this box's tmux 3.5a
// and types the literal string "WheelUpPane", so tmux cannot synthesise the event either.
function wsWheel(dir, n) {
  if (!viewIsTui || !liveWs || liveWs.readyState !== WebSocket.OPEN) return;
  void n;                                   // notches no longer scale the send: one page per flick
  wsScrollKey(dir === 'up' ? 'PPage' : 'NPage');
  if (dir === 'up') { viewAppScrolled = true; _setScrolled(true); }
}
...
function buildViewTerminal(name) {
  ...
  viewLastCH = 0; viewLineCache = [];   // 13a: fresh pre element -> empty diff baseline
  const container = document.getElementById('terminal-body');
  ...
  viewCapture = ta;
  // Clear ONLY once the keystrokes have actually left the browser — otherwise a
  // stalled socket eats what was typed and leaves no trace of it anywhere.
...
function connectView(name) {
  ...
  liveWs = ws;
  // A fresh socket knows nothing yet: seqs from the old one can never be echoed
  // on this one, and ack support is re-learned from this server's meta frame.
  viewAckSupported = false;
  armKeepalive(ws);
  armLiveWatch();
  ws.onopen = () => { if (liveWs !== ws) return; reconnectDelay = 600; setTermState('connected'); };
  ws.onmessage = (e) => {
    if (liveWs !== ws) return;      // superseded socket: its frames are not ours
    liveTouch();                    // ANY frame (content/meta/hb/pong) = alive
    ...
    if (msg.type === 'content') {
      ...
      viewIsTui = !!msg.tui;
      // Delivery receipts are used only when THIS server says it sends them.
      // Assuming them against an older server would fail every send on timeout.
      viewAckSupported = !!msg.ack;
      ...
      // The outcome, straight from the process that ran send-keys.
      const entry = uplink && uplink.settle(msg.seq);
      if (!entry) return;           // late echo for something already given up on
      if (msg.ok) onSendOk(entry.meta);
      else onSendFailed(entry.meta, msg.err || 'server could not deliver it');
    }
    else if (msg.type === 'pong') {
      // Keepalive echo — proves the UPLINK, which no inbound content frame can.
      if (uplink && typeof msg.seq === 'number') uplink.settle(msg.seq);
    }
    // 'hb' carries no payload — the liveTouch() above is its whole job.
  };
  ws.onclose = () => { if (liveWs !== ws) return; setTermState('disconnected'); scheduleReconnect(); };
  ws.onerror = () => { if (liveWs !== ws) return; setTermState('error'); };
}
...
function closeTerminal() {
  ...
  if (viewAppScrolled && liveWs && liveWs.readyState === WebSocket.OPEN) {
    for (let i = 0; i < 16; i++) wsScrollKey('NPage');
  }
  ...
  disarmLiveWatch();
  if (viewLatch) { viewLatch.clear(); viewLatch = null; }
  ...
  viewAppScrolled = false; viewStickBottom = true; viewLineCache = [];
...
//
// EVERY send is verified end to end (2026-09-10). Before this, sendText/wsKey
// returned silently whenever readyState was not OPEN, and composeSend cleared
// the textarea regardless — so a message that never left the browser looked
// EXACTLY like one that was delivered. That is the same mistake the 08-15 fix
// documented for the downstream direction: judging health by a proxy
// (readyState) instead of the outcome (the server says it ran it).
//
// Three gates now stand between a keystroke and a false success:
//   1. socket state — an unusable socket fails LOUDLY to the caller;
//   2. bufferedAmount — bytes queued in the browser that the socket is not
//      draining mean a stalled uplink, which readyState never reveals;
//   3. a server ack for the seq we sent (see UplinkTracker + _ackSweep).
const SEND_BUFFER_MAX = 512 * 1024;   // queued-but-undrained bytes = stalled uplink

// Raw frame write. Returns true only if the browser actually accepted it.
function _wsWrite(obj) {
  if (!liveWs || liveWs.readyState !== WebSocket.OPEN) return false;
  if (liveWs.bufferedAmount > SEND_BUFFER_MAX) {
    setTermState('uplink stalled — reconnecting');
    forceReconnect();
    return false;
  }
...
// Write a frame and, when the server supports receipts, book it for ack
// tracking. `meta` comes back to us if the round trip is lost.
// Returns true when the frame left the browser (NOT that it was delivered —
// delivery is settled later, by ack or by _ackSweep).
function _wsSendTracked(obj, meta) {
  if (!uplink || !viewAckSupported) return _wsWrite(obj);
  const n = uplink.track(meta);
  const sent = _wsWrite(Object.assign({ seq: n }, obj));
  if (!sent) uplink.settle(n);   // never left: not an in-flight round trip
  return sent;
}
...
function sendText(text, meta) {
  ...
  if (liveMode === 'view' || liveMode === 'reader') {
    ok = _wsSendTracked({ type: 'keys', data: text }, meta || { kind: 'text' });
  } else {
    ok = _wsWrite({ type: 'input', data: text });
    if (ok && liveTerm) liveTerm.focus();
  }
  if (ok) _viewSnapLive();   // typing snaps the view back to the live tail (input line)
  return ok;
}

// Raw named-key send (no scroll-state side effect) — used by viewScroll.
function wsKey(key, meta) {
  if (liveMode === 'view' || liveMode === 'reader') {
    return _wsSendTracked({ type: 'key', data: key }, meta || { kind: 'key', key: key });
  }
  const seq = PTYSEQ[key];
  if (seq == null) return false;
  const ok = _wsWrite({ type: 'input', data: seq });
  if (ok && liveTerm) liveTerm.focus();
  return ok;
}
...
// Transcript paging is telemetry, not a message: fire-and-forget, never tracked.
// closeTerminal alone fires 16 of these, and a burst that the closing socket
// never acks must not read as 16 lost round trips.
function wsScrollKey(key) {
  if (liveMode === 'view' || liveMode === 'reader') return _wsWrite({ type: 'key', data: key });
  return wsKey(key);
}
...
function applyComposeMode() {
  ...
  if (!bar) return;
  const on = composeMode && !!liveMode;
  bar.classList.toggle('show', on);
  if (!on) composeNote('', null);   // a stale verdict must not outlive its bar
}
...
// ── Compose delivery state ──────────────────────────────────────────────────
// A composed message is TWO frames on the wire: the literal text, then Enter.
// They are tracked as one unit so a partial delivery is reported for what it
// is. Before 2026-09-10 the box was cleared unconditionally the instant the
// button was pressed, so a message that never reached the server was
// indistinguishable from one that did — the operator's only signal was that
// nothing happened, and the text was gone.
function composeNote(text, cls) {
  const bar = document.getElementById('compose-bar');
  const note = document.getElementById('compose-note');
  if (bar) { bar.classList.remove('pending', 'err'); if (cls) bar.classList.add(cls); }
  if (note) { note.textContent = text || ''; note.classList.toggle('show', !!text); }
}
// Put a failed message back where the operator typed it — unless they have
// already started typing something else, in which case clobbering would be a
// second, worse kind of data loss. It is in composeHist either way, so ↑ recalls it.
function composeRestore(text, why) {
  const inp = composeInput();
  if (inp && !inp.value) { inp.value = text; composeGrow(); }
  composeNote('not sent (' + why + ') — retry', 'err');
  if (navigator.vibrate) navigator.vibrate([25, 40, 25]);
}
function onSendOk(meta) {
  if (!meta || meta.kind !== 'compose') return;
  meta.group[meta.part] = true;
  if (meta.group.text === true && meta.group.enter === true) composeNote('', null);
}
function composeSend() {
  const inp = composeInput(); if (!inp) return false;
  ...
  if (!text) return sendNamed('Enter');   // empty send = bare Enter

  const group = { text: null, enter: null };
  // The text MUST leave the browser before the Enter does. If it does not,
  // sending a bare Enter would submit whatever the pane's input line already
  // held — so bail with the message intact instead.
  if (!sendText(text, { kind: 'compose', part: 'text', text: text, group: group })) {
    composeRestore(text, 'not connected');
    return false;
  }
  // "sending…" is a promise that an ack will resolve. Against a server that
  // does not send receipts nothing ever would, so it must not be shown at all —
  // a status that can never clear is worse than none.
  composeNote(viewAckSupported ? 'sending…' : '', viewAckSupported ? 'pending' : null);
  if (!wsKey('Enter', { kind: 'compose', part: 'enter', text: text, group: group })) {
    // Text is on its way but the submit is not: restoring here would duplicate
    // it in the pane. Say exactly that, and leave the box clear.
    composeNote('text sent, submit failed — press ↵', 'err');
  }
  _viewSnapLive();
...
document.addEventListener('visibilitychange', () => {
  if (liveWatch) liveWatch.setHidden(document.hidden);
  if (document.hidden) return;
  if (liveMode && liveName && liveSocketDead()) {
    ...
    // The socket LOOKS alive — but a frozen tab may have swallowed the pending
    // paint callback, so drop the latch and repaint the newest frame we hold.
    // (reader rides the view socket too — it was excluded here before, 08-15.)
    if (viewLatch) viewLatch.clear();
    if (viewPending == null && viewLastFrame != null) viewPending = viewLastFrame;
    if (viewPending != null) _viewSchedule();
    // Whether the stream itself survived the background is decided by the stall
    // watchdog on its next poll (after its grace window), not guessed here.
  }
...
window.addEventListener('pageshow', () => {
  if (!liveMode || !liveName) return;
  if (liveWatch) liveWatch.setHidden(false);
  if (liveSocketDead()) { reconnectLive(); return; }
  // bfcache restore: the document (and any pending rAF callback) was frozen.
  if (viewLatch) viewLatch.clear();
  if (viewPending == null && viewLastFrame != null) viewPending = viewLastFrame;
  if (viewPending != null) _viewSchedule();
});
...
// clip per line, and returns one {cls, inner} per emitted row (renderAnsiToLines
// joins them into HTML). Reuses parseAnsiToSegments so the (correct) ANSI
// decoding isn't duplicated. The list form lets the capture view DOM-diff frames
// (13a) instead of rebuilding innerHTML every capture.
...
function renderAnsiToLineList(ansiText) {
  const segments = parseAnsiToSegments(ansiText);
  const lines = [[]];
  for (const seg of segments) {
  ...
  for (let i = 0; i < rows.length; i++) {
    if (skip.has(i)) {
      if (i === todoSummaryAt)
        out.push({ cls: 'tline todo-sum',
                   inner: `☐ ${todoCount} todo${todoCount > 1 ? 's' : ''} hidden · hold esc` });
      continue;
    }
    out.push({ cls: rows[i].cls, inner: rows[i].inner || '&nbsp;' });
  }
  return out;
}

// String form: join the per-line list into HTML. Used by innerHTML callers (the
// reader's pinned status line renders a captured frame's status block this way).
function renderAnsiToLines(ansiText) {
  return renderAnsiToLineList(ansiText)
    .map(r => `<div class="${r.cls}">${r.inner}</div>`).join('');
}

function openTiledView() {
  document.getElementById('tile-picker').classList.add('open');
  fetch('/api/status').then(r => r.json()).then(data => {
    // Tiles stream a pane's capture — a pane-less bg job has nothing to tile.
    allSessions = data.sessions.filter(s => s.command === 'claude' && s.target).sort(favStateSort);
